...
public class DbUnitUtils {
     ...
     * 插入XML数据文件到H2数据库.
     */
    public static void appendData(BasicDataSource h2DataSource, String... xmlFilePaths) throws Exception {
     ...
     */
    public static void removeData(BasicDataSource h2DataSource, String... xmlFilePaths) throws Exception {
     ...
     * 在H2数据库中删除XML表示的数据.
     */
    public static void removeDataFromXML(BasicDataSource h2DataSource, String... xmlFilePaths) throws Exception {
    ...
    private static void execute(DatabaseOperation operation, DataSource h2DataSource, String... xmlFilePaths)
        ...
        if (h2DataSource instanceof BasicDataSource) {
            BasicDataSource basicDataSource = (BasicDataSource) h2DataSource;
            String className = basicDataSource.getDriverClassName();
            if (className.equalsIgnoreCase("oracle.jdbc.driver.OracleDriver")) {
                connection = new OracleConnection(h2DataSource.getConnection(), h2DataSource.getConnection().getMetaData().getUserName());
            } else {
                connection = new H2Connection(h2DataSource.getConnection(), "");
            }
        } else {
            connection = new H2Connection(h2DataSource.getConnection(), "");
        }
        ...
        for (String xmlPath : xmlFilePaths) {
            try {
                ...
                operation.execute(connection, rDataSet);
            } catch (IOException e) {
